...
values ('Nome', 'Cognome', '2023-01-01', 'nome.cognome@email.it', 'strongpwd',
...
values ('Call of duty 9',
        ...
        (select id
         from developer d
         where d.software_house_id = (select id from software_house sh where sh.name = 'Activision')
           and d.specialty_id = (select id from specialty s where s.name = 'GAME DEVELOPER')),
        (select id
         from developer d
         where d.software_house_id = (select id from software_house sh where sh.name = 'Activision')
           and d.specialty_id = (select id from specialty s where s.name = 'AUDIO DEVELOPER')),
        (select id
         from developer d
         where d.software_house_id = (select id from software_house sh where sh.name = 'Activision')
           and d.specialty_id = (select id from specialty s where s.name = 'GRAPHIC DEVELOPER')));
...
from videogame v
         ...
         join platform p on v.platform_id = p.id
where v.genre_id = (select genre_id from videogame v2 where v2.id = :videogame_id)
  and v.platform_id = (select platform_id from videogame v3 where v3.id = :videogame_id)
  and v.id <> :videogame_id;
...
where p.id = :purchase_id
  and p.status_id = (select s.id from status s where s.name = 'IN LAVORAZIONE');
