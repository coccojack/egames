...
const Products = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      setLoading(true);
      const response = await fetch("http://localhost:8081/egames/videogame/all");
      if (componentMounted) {
  ...
  const filterProduct = (cat) => {
    const updatedList = data.filter((item) => item.platform.name === cat);
      ...
      <div className="container my-3 py-3">
        <div className="row">
          <div className="col-12">
            <h2 className="display-5 text-center">Trending Games</h2>
            <hr />
          </div>
        </div>
